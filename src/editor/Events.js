...
class Events {
    ...
    loadData() {
        const filename1 = location.pathname.replace('/editor.html', '')
        let filename = location.search.replace('?', '') || 'Demo.json'
        ...
        fetch('/files/' + filename)
            ...
            .then(data => {
                ...
                this.nodeHandler.container.querySelectorAll('*')[0].click()
            })
            .catch(()=>{
    ...
    save() {
        // const filename1 = location.pathname.replace('/editor/', '')
    ...
    nodeTypeButtons() {
        const types = 'h1 h2 p html css js go sql'.split(' ')
        types.forEach(type => {
